...
from lua_annotations.build_process import (
...
def convert_dict(
    ...
    def table_block(entry_lines: list[str], level: int) -> str:
        ...
        outer = INDENT * level
        return '\n'.join(['{', *(f'{inner}{line}' for line in entry_lines), f'{outer}}}'])
    ...
    import_lines = resolver.get_import_lines() if include_imports else []
...
class LuaPathResolver:
    ...
    def _resolve_expr(self, env: Environment, raw_expr: str, inline_require: bool = False):
        ...
        resolved = raw_expr.replace(':', f'{map[env]}.', 1)
        return self._collapse_dots(resolved)
            ...
            for workdir, lua_expr in path_map.items():
                try:
                    ...
                    return (
                        env,
                        relative,
                        self._resolve_expr(env, lua_expr, inline_require),
                    )
                except ValueError:
                    continue
...
class LuaPath:
    ...
    def _append_parts(self, expr: str, parts: list[str]) -> str:
        for part in parts:
            accessor = self._accessor(part)
            if expr.endswith('.') and accessor.startswith('.'):
                expr += accessor[1:]
            else:
                expr += accessor
        return expr
    ...
    def to_lua_relative(self, resolver: LuaPathResolver | None = None):
    ...
    def to_lua(self, resolver: LuaPathResolver, inline_require: bool = False):
        if self.relative:
            return self.to_lua_relative(resolver)
