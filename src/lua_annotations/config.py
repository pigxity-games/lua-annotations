...
class Config:
    ...
    def from_raw(cls, data: Any):
        ...
        workspace_common = _parse_environment_maps(
            raw_workspace_common, 'workspace_common', require_all=False
        )

        raw_workspaces = _parse_list_field(data.get('workspaces', []), 'workspaces')
        workspaces = [
            WorkspaceConfig.from_raw(raw, i, workspace_common)
            for i, raw in enumerate(raw_workspaces)
        ]
...
def read_config(config_file: Path):
    ...
    try:
        ...
        raise ConfigParseError(
            f'Invalid JSON in {config_file}: line {exc.lineno}, column {exc.colno}: {exc.msg}'
        ) from exc
