# Components
Unlike services, components represent **individual Roblox instances** which are bound by CollectionService tags. They usually use services.

## Simple components (`@bindTag`)
For simplicity, we'll start with the `@bindTag` annotation, which could be considered as a "simple component" of some sorts. It binds to a function which is called for each tagged instance. This is good for simple objects that you have a lot of, such as obby killbricks.
...
!!! note
    ...
    To inject a service, simply do so like another service. Note that you cannot inject components into services, due to their multi-instanced nature. If you would like to create components programmatically, you should add a tag to the instance. 

The first argument to `@component` is the tag name. It may also be a list of tags:

```lua
--@component, [Counter, ClickCounter]
local counter = {}

return counter
```

The first tag is considered the main tag and is used when another component dependency needs to be created automatically.
...
## The `data` argument
You may use the `data` kwarg to specify a "component registry". When a component is created, it uses data from this table to create it. This allows for specifying initial data or for easy access of components from elsewhere.
...
!!! note
    ...
    * `typegen=registry` generates a registry-shaped type in `ServiceTypes.lua`.
...
```lua
--@service, depends=[CounterRegistry]
local service = {}
